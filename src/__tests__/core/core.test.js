...
// Functions
const runTest = (test, subtestNames: Array<string>) => {
    const { testName } = test;
        ...
        if (!hasSubtests) {
            ...
            if (subtestNames.length > 0) {
                ...
                subtestNames.forEach(subtestName => {
                    ...
                    const specNames = testPayloads.map(p => specName);

                    startTestingPayloads(testPayloads, expectedResponses, specNames);
                });
            } else {
                // No subtests were specified but the test has subtests - run them all
                for (let k in test.subtests) {
...
const startTestingPayloads = (testPayloads, expectedResponses, specNames) => {
