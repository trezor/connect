...
import type { KinAddress } from '../../types/kin';
...
export default class KinGetAddress extends AbstractMethod {
    ...
    constructor(message: CoreMessage) {
        ...
        // set info
        if (bundle.length === 1) {
            this.info = `Export Kin address for account #${ (fromHardened(this.params[0].path[2]) + 1) }`;
        } else {
            this.info = 'Export multiple Kin addresses';
        }
    }
    ...
    async run(): Promise<KinAddress | Array<KinAddress>> {
        const responses: Array<KinAddress> = [];
        ...
        for (let i = 0; i < this.params.length; i++) {
            ...
            if (batch.showOnTrezor) {
                const silent = await this.device.getCommands().kinGetAddress(
                    batch.path,
                    false
                );
            ...
            const response = await this.device.getCommands().kinGetAddress(
                batch.path,
                batch.showOnTrezor
            );
            responses.push({
