...
echo "sync "$SOURCE" with "$BUCKET"/connect"
...
if [ "x$3" == "x-clear" ]; then
    aws s3 sync --delete --cache-control 'public, max-age=3600' $SOURCE s3://$BUCKET/connect
else
    aws s3 sync --cache-control 'public, max-age=3600' $SOURCE s3://$BUCKET/connect
fi

aws cloudfront create-invalidation --distribution-id $DISTRIBUTION_ID --paths '/connect/*'
