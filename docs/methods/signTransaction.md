...
* `refTxs` - *optional* `Array` of [RefTransaction](../../src/js/types/trezor.js#L148-L157). Referenced transactions are required to sign non-segwit transaction. If you don't want to use build-in `blockbook` backend you can optionally provide those data from your own backend transformed to `Trezor` format. For Segwit transactions this field should be an empty `Array`.
