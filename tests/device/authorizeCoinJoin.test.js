// @flow
// https://github.com/trezor/trezor-firmware/blob/master/tests/device_tests/bitcoin/test_authorize_coinjoin.py
...
describe('TrezorConnect.authorizeCoinJoin', () => {
    ...
    conditionalTest(['1', '<2.4.4'], 'Coinjoin success', async () => {
        ...
        // remove all button listeners from initTrezorConnect
        // DebugLink decision should not be required from now on
        ...
        const params = {
            ...
            refTxs: TX_CACHE(['e5b7e2', 'f982c0'], true), // force tx cache because of fake input
            coin: 'testnet',
            preauthorized: true,
        };

        const signedTx = await TrezorConnect.signTransaction(params);
        expect(signedTx.payload).toMatchObject({
            serializedTx:
        ...
        // sign again ...
        const signedTx2 = await TrezorConnect.signTransaction(params);
        expect(signedTx2.success).toBe(true);

        // ... and again, fees should exceed maxRounds
        const signedTx3 = await TrezorConnect.signTransaction(params);
        expect(signedTx3.success).toBe(false);
        expect(signedTx3.payload).toMatchObject({ error: 'Exceeded number of CoinJoin rounds.' });
    });
});
